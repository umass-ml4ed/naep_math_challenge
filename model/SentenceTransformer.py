...
from utils.analysis import analysis_label_embedding
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar, position=0):
            ...
            if retriever is not None and epoch != 0 and epoch % checkpoint_save_steps == 0:
                    ...
                    for key, qdf in examples.items():
                        key_name = var.QUESTION_TO_NAME[key]
                        analysis_label_embedding(qdf, key=key,
                                                 title='embedding space for {} at epcoh {}'.format(key_name, epoch),
                                                 path=checkpoint_path + f'/{key_name}_figure_{epoch}')
